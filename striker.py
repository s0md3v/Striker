...
def sqli(url):
    ...
    if match:
        ...
        option = raw_input(
            "%s Would you like to see the whole report? [Y/n] " % que
        ).lower()
        if option == "n":
            pass
        else:
    ...
    try:
        ...
        if detect:
            ...
            if "WordPress" in detect:
                option = raw_input(
                    "%s Would you like to use WPScan? [Y/n] " % que
                ).lower()
                if option == "n":
                    pass
                else:
            ...
            option = raw_input("%s Would you like to use WPScan? [Y/n] " % que).lower()
            if option == "n":
                pass
            else:
                os.system("wpscan --random-agent --url %s" % domain)
...
def honeypot(ip_addr):
    ...
    honey = (
        "https://api.shodan.io/labs/honeyscore/%s?key=C23OXE0bVMrul2YeqcL7zxb6jZ4pj2by"
        % ip_addr
    )
    try:
...
def bypass(domain):
    ...
    result = br.open("http://www.crimeflare.info/cgi-bin/cfsearch.cgi ", post).read()

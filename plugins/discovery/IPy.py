...
class IPint:
    ...
    def __init__(self, data, ipversion=0):
        ...
        # handle IP instance as an parameter
            ...
            if not _checkNetaddrWorksWithPrefixlen(
                self.ip, self._prefixlen, self._ipversion
            ):
                raise ValueError(
    ...
    def _printPrefix(self, want):
        ...
        if want:
            if want == 2:
                ...
                if not isinstance(netmask, types.IntType) and not isinstance(
                    netmask, types.LongType
                ):
                    netmask = netmask.int()
    ...
    def strFullsize(self, wantprefixlen=None):
        ...
        return intToIp(self.ip, self._ipversion).lower() + self._printPrefix(
            wantprefixlen
        )
...
def parseAddress(ipstr):
    ...
    if ipstr.find(":") != -1:
        ...
        if len(hextets) < 8:
            ...
            if (
                hextets.index("") < len(hextets) - 1
                and hextets[hextets.index("") + 1] == ""
            ):
                hextets.remove("")
            # catch '::'
            if (
...
def intToIp(ip, version):
    ...
    if version == 4:
        if ip > 0xFFFFFFFF:
            raise ValueError(
                "IPv4 Addresses can't be larger than 0xffffffff: %s" % (hex(ip))
            )
        for l in range(4):
            ret = str(ip & 0xFF) + "." + ret
            ip = ip >> 8
...
def _checkNetmask(netmask, masklen):
    ...
    while bits > 0:
        if (num & 1) == 0:
            raise ValueError(
                "Netmask %s can't be expressed as an prefix." % (hex(netmask))
            )
        num = num >> 1
        bits -= 1

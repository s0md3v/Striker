...
class BarGraph:
    ...
    def build_legend(self, barColors):
        ...
        for color in barColors:
            ...
            legend += (
                "<td>"
                + self.build_bar("", self.barWidth, self.barWidth, color)
                + "</td>"
            )
    ...
    def create_hBar(
        ...
        if percent < 0:
            ...
            bar += self.build_bar(
                value, int(round(percent * mul)), self.barWidth, bColor
            )
            ...
            if percent:
                ...
                bar += (
                    "<td width=1 height=" + str(self.barWidth + (border * 2)) + "></td>"
                )
            bar += (
                '<td style="'
                + self.__cssPERCVALUES
    ...
    def create_vBar(
        ...
        if percent < 0:
            ...
            bar += (
                "<td height="
                + str(spacer)
                + '></td></tr><tr align=center valign=top><td style="'
                + self.__cssLABELBG
                + '">'
            )
            bar += self.build_bar(
                value, self.barWidth, int(round(percent * mul)), bColor
            )
    ...
    def create(self):
        ...
        drc = (
            hasattr(self.barColors, "split")
            and self.barColors.split(",")
            or self.barColors
        )
        val = []
        bc = []
        ...
        graph = (
            ...
            + str(self.graphPadding)
            + "><tr>"
        )
        graph += (
            "<td" + (self.__cssGRAPH and ' style="' + self.__cssGRAPH + '"' or "") + ">"
        )
        ...
        for v in val:
            ...
            if self.type == "hbar":
                ...
                if titleLabel != "":
                    graph += (
                        '<td style="' + self.__cssTITLE + '">' + titleLabel + "</td>"
                    )
                    ...
                    if m or not i:
                        ...
                        if self.type == "fader":
                            graph += self.build_fader(
                                value,
                                int(round(self.barWidth / 2)),
                                self.barWidth,
                                int(round(percent * mul)),
                                bColor,
                            )
                        else:
                            graph += self.build_bar(
                                value, int(round(percent * mul)), self.barWidth, bColor
                            )
                        graph += (
                            "</td><td width="
                            + str(int(round((100 - percent) * mul)))
                            + "></td>"
                        )
                        graph += "</tr></table></td>"
                        if self.showValues < 2:
        ...
        if self.debug:
            graph += "<br>sum=%s max=%s max_neg=%s max_dec=%s " % (
                sum,
                max,
                max_neg,
                max_dec,
            )
            graph += "mPerc=%s mPerc_neg=%s mul=%s valSpace=%s" % (
                mPerc,
                mPerc_neg,
                mul,
                valSpace,
